...
class Database:
    ...
    async def get_banned(self):
        ...
        lz_verified = self.grp.find({'chat_status.is_lazy_verified': True})
    ...
    async def get_settings(self, id):
        ...
        #
        # NOTE: when adding new toggles to the bot configuration, be sure to
        # include a sensible default here.  Without a default, new settings
        # won't be available until the group is manually updated, which can
        # lead to KeyError exceptions when reading them later.
        default = {
            ...
            # Whether to notify admins/channel when a user issues a request via
            # the request flow.  Default is True so that requests are delivered
            # unless explicitly disabled by an admin.
            'request_notify': True
        }
