...
lock = asyncio.Lock()
# The `send_not_found_suggestions` helper is defined below in this module.  Do
# not re-import it from this file, as that can lead to redundant imports.
...
# Cache for not-found suggestions.
# Telegram limits callback_data to 64 bytes, so we can't safely embed full
# movie/file names there. We store suggestions in-memory and reference them
# by a short cache key.
SUGGESTIONS_CACHE = {}

# ---------------------------------------------------------------------------
# Not found suggestion utilities
#
# When a user searches for a file name and no direct match is found in the
# database, we provide a list of close matches based on a simple regex of the
# first word in their query.  The user can click on any of the suggested
# titles to perform a new search.  A "Request" button is also provided to
# allow users to signal that their desired file isn't available.  Whether
# clicking the request button sends a notification to admins or the request
# channel is controlled by the per‑group `request_notify` setting.

async def get_suggestions(chat_id: int, query: str, file_type: str = None, limit: int = 5):
    """Return a list of up to ``limit`` suggested file names similar to ``query``.

    The suggestions are based on performing a broad match against the first
    word of the query.  Duplicate names are filtered out.  If no
    suggestions are found, an empty list is returned.

    :param chat_id: ID of the chat for which to search.  (Unused currently
        but kept for future per‑chat customisation.)
    :param query: The user's search string.
    :param file_type: Optional file type to restrict results.
    :param limit: Maximum number of suggestions to return.
    :return: A list of file name strings.
    """
    suggestions = []
    # Normalise and split the query.
    q = (query or "").strip()
    if not q:
        return suggestions
    # Use the first token in the query for a broad match.  This yields a
    # generous set of candidates without returning the entire database.
    word = q.split()[0]
    # Perform a regex match on the beginning of each file name.  get_bad_files
    # returns all results for the given pattern, sorted by recency.
    try:
        files, total = await get_bad_files(word, file_type)
    except Exception:
        return suggestions
    # Collect unique file names until the limit is reached.
    for file in files:
        name = str(file.file_name).strip()
        if not name:
            continue
        # Avoid duplicate suggestions (case insensitive).
        if any(name.lower() == s.lower() for s in suggestions):
            continue
...
async def send_not_found_suggestions(client: Client, msg, query: str, settings: dict):
    """Send a not‑found message with suggestions and a request button.

    Called when a search yields no results.  This helper obtains
    suggestions, builds an inline keyboard with suggestion buttons and a
    request button, and replies to the user.  The message auto‑deletes
    after the configured delay if auto_delete is enabled.

    :param client: The Pyrogram client.
    :param msg: The message object representing the user's search.
    :param query: The normalised search string.
    :param settings: The current group settings.
    """
    chat_id = msg.chat.id
    suggestions = await get_suggestions(chat_id, query)
    buttons = []

    # Build suggestion buttons.
    # IMPORTANT: Telegram limits callback_data to 64 bytes, so do NOT embed
    # the full suggestion string there. Store suggestions in-memory and refer
    # to them with a short cache key.
    user_id = msg.from_user.id if msg.from_user else 0
    cache_key = f"{user_id}:{int(time.time())}:{secrets.token_hex(3)}"
    ...
    for i, suggestion in enumerate(suggestions[:5]):
        # Button text can be long; keep it readable.
        btn_text = suggestion if len(suggestion) <= 50 else suggestion[:47] + "…"
        buttons.append([
            InlineKeyboardButton(
                text=btn_text,
                callback_data=f"suggest#{user_id}#{cache_key}#{i}"
            )
        ])

    # Always add a request button.
    buttons.append([
        InlineKeyboardButton(
            text="Request 📩",
            callback_data=f"req#{user_id}#{cache_key}"
        )
    ])
    # Compose the not‑found message.  If no suggestions are available, fall
    # back to a generic message.
    if suggestions:
        text = f"**No results found for:** `{query}`\n\n" \
               f"Maybe you meant one of these:"
    else:
        text = f"**No results found for:** `{query}`"
    try:
        ...
        # If something goes wrong (e.g. invalid button data), log it so it
        # shows up in Render logs.
        print(f"NOT_FOUND_SUGGESTIONS_SEND_FAILED: {type(e).__name__}: {e}")
        return
    # Honour the auto_delete setting.  If enabled, remove the suggestion
    # message after 150 seconds to keep the chat tidy.
    try:
        if settings.get('auto_delete'):
            await asyncio.sleep(150)
            await reply.delete()
            # Also remove the user's search message if possible.
            try:
                await msg.delete()
            except Exception:
                pass
    except KeyError:
        # If the setting is missing, enable auto_delete by default for the
        # group to maintain existing behaviour.  The initial call will
        # populate the key so subsequent lookups succeed.
...
async def give_filter(client, message):
    """Entry point for handling incoming text messages in both groups and PMs.

    This handler orchestrates the flow through global filters, manual filters and
    the auto‑filter search logic.  Previously, the auto‑filter would only be
    invoked if the per‑group ``auto_ffilter`` setting was enabled.  However,
    this meant that useful features such as spell‑check suggestions and the
    new not‑found suggestion flow were never reached unless admins explicitly
    toggled the setting on.  To provide a more consistent user experience,
    the auto‑filter is now invoked whenever there are no matching manual or
    global filters, regardless of the current ``auto_ffilter`` setting.  The
    setting still controls whether search results are automatically sent, but
    the not‑found suggestions will always appear when appropriate.

    The original logic for the support chat is preserved: in the support
    group, the bot simply reports the number of results rather than sending
    files.
    """
    # Skip support chat where search isn't allowed.
    if message.chat.id == SUPPORT_CHAT_ID:
        # In support chat, inform the user about available results without
        # performing the full auto‑filter flow.
        search = message.text
        ...
        if total_results == 0:
            ...
            return await message.reply_text(
                text=(
                    f"<b>Hᴇʏ {message.from_user.mention}, {str(total_results)} ʀᴇsᴜʟᴛs ᴀʀᴇ ғᴏᴜɴᴅ ɪɴ ᴍʏ "
                    f"ᴅᴀᴛᴀʙᴀsᴇ ғᴏʀ ʏᴏᴜʀ ᴏ̨ᴜᴇʀʏ {search}. Kɪɴᴅʟʏ ᴜsᴇ ɪɴʟɪɴᴇ sᴇᴀʀᴄʜ ᴏʀ ᴍᴀᴋᴇ "
                    f"ᴀ ɢʀᴏᴜᴘ ᴀɴᴅ ᴀᴅᴅ ᴍᴇ ᴀs ᴀᴅᴍɪɴ ᴛᴏ ɢᴇᴛ ᴍᴏᴠɪᴇ ғɪʟᴇs. Tʜɪs ɪs ᴀ "
                    f"sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ sᴏ ʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ғɪʟᴇs ғʀᴏᴍ ʜᴇʀᴇ...\n\n"
                    f"Fᴏʀ Mᴏᴠɪᴇs, Jᴏɪɴ @free_movies_all_languages</b>"
                ),
                parse_mode=enums.ParseMode.HTML
            )

    # For all other chats, run through global and manual filters first.
    # If either returns a truthy value, they have sent a reply and no further
    # processing is needed.  If both return False, invoke the auto filter.
    glob = await global_filters(client, message)
    if glob is not False:
        return
    manual = await manual_filters(client, message)
    if manual is not False:
        return

    # Retrieve settings; auto_ffilter still controls whether full auto
    # responses are sent, but auto_filter is always invoked to trigger
    # spell‑check or not‑found suggestions.  We wrap in a try/except to
    # recover missing keys gracefully.
    try:
        settings = await get_settings(message.chat.id)
    except Exception:
        settings = {}
    # Always invoke auto_filter when nothing else matched.  Pass through
    # regardless of the auto_ffilter setting so that at minimum the
    # not‑found suggestion flow runs.  The auto_filter function itself
    # respects settings['auto_ffilter'] for sending actual results.
    try:
        await auto_filter(client, message)
    except Exception as e:
        logger.error(f"Auto filter execution failed: {e}")
...
async def next_page(bot, query):
    ...
    if not files:
        # No more files on subsequent pages.  Rather than silently
        # returning, offer the user suggestions for similar titles and a
        # request button.  This improves the UX when the user reaches
        # the end of the result set but still wants more options.
        try:
            settings = await get_settings(query.message.chat.id)
        except Exception:
            settings = {}
        # Use the last known search term for suggestions.  If none is
        # available, just abort.
        if search:
            await send_not_found_suggestions(bot, query.message, search, settings)
        return
    settings = await get_settings(query.message.chat.id)
    ...
    if ENABLE_SHORTLINK and settings['button']:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"[{get_size(file.file_size)}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('Linkz') and not x.startswith('{') and not x.startswith('Links') and not x.startswith('www'), file.file_name.split()))}", url=await get_shortlink(query.message.chat.id, f"https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}")
                ),
            ]
            for file in files
        ]
        ...
        btn = [
            [
                InlineKeyboardButton(
                    text=f"{' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('Linkz') and not x.startswith('{') and not x.startswith('Links') and not x.startswith('www'), file.file_name.split()))}", url=await get_shortlink(query.message.chat.id, f"https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}")
                ),
                InlineKeyboardButton(
                    text=f"{get_size(file.file_size)}",
                    url=await get_shortlink(query.message.chat.id, url=f"https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}")
                ),
            ]
            for file in files
        ]
    ...
    if files:
        ...
        if ENABLE_SHORTLINK and settings['button']:
            ...
            btn = [
                [
                    InlineKeyboardButton(
                        text=f"{' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('Linkz') and not x.startswith('{') and not x.startswith('Links') and not x.startswith('www'), file.file_name.split()))}",
                        url=await get_shortlink(query.message.chat.id, url=f"https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}")
                    ),
                    InlineKeyboardButton(
                        text=f"{get_size(file.file_size)}",
        ...
        return await query.answer(f"Sᴏʀʀʏ, Nᴏ ғɪʟᴇs ғᴏᴜɴᴅ ғᴏʀ ʏᴏᴜʀ {movie}.", show_alert=True)
...
async def select_language(bot, query):
    ...
    btn = [[
        ...
        InlineKeyboardButton("Gᴜɪᴀʀᴀᴛɪ", callback_data=f"lang#{userid}#guj"),
        InlineKeyboardButton("Mᴀʀᴀᴛʜɪ", callback_data=f"lang#{userid}#mar"),
        InlineKeyboardButton("Pᴜɴɪᴀʙɪ", callback_data=f"lang#{userid}#pun")
    ],[
...
async def select_quality(bot, query):
    ...
    if int(userid) not in [query.from_user.id, 0]:
        return await query.answer(script.ALRT_TXT.format(query.from_user.first_name), show_alert=True)
    btn = [[
        InlineKeyboardButton("Sᴇʟᴇᴄᴛ Yᴏᴜʀ Dᴇꜱɪʀᴇᴅ Lᴀɴɢᴜᴀɢᴇ ↓", callback_data=f"lusifilms#{userid}#unknown")
    ],[
...
async def seasons_check(bot, query):
    ...
    if int(userid) not in [query.from_user.id, 0]:
        return await query.answer(script.ALRT_TXT.format(query.from_user.first_name), show_alert=True)
    if seasons == "unknown":
        return await query.answer("Sᴇʟᴇᴄᴛ ᴀɴʏ Season ғʀᴏᴍ ᴛʜᴇ ʙᴇʟᴏᴡ ʙᴜᴛᴛᴏɴs !", show_alert=True)
    movie = temp.KEYWORD.get(query.from_user.id)
    cap = CAP.get(movie)
    if not movie:
        return await query.answer(script.OLD_ALRT_TXT.format(query.from_user.first_name), show_alert=True)
...
async def select_seasons(bot, query):
    ...
    if int(userid) not in [query.from_user.id, 0]:
        return await query.answer(script.ALRT_TXT.format(query.from_user.first_name), show_alert=True)
    btn = [[
        InlineKeyboardButton("Sᴇʟᴇᴄᴛ Yᴏᴜʀ Dᴇꜱɪʀᴇᴅ Lᴀɴɢᴜᴀɢᴇ ↓", callback_data=f"seasons#{userid}#unknown")
    ],[
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
            InputMediaPhoto("https://te.legra.ph/file/e883c4a1e58c241d5565c.jpg")
        ) 
        await query.message.edit_text(
        ...
        """
        Toggle one of the group settings from the inline settings panel.

        The callback data is formatted as:
            setgs#<setting_type>#<current_status>#<group_id>

        Using the group id from the callback ensures the correct target even
        when the user is in a private chat and has no active connection.
        """
        ident, set_type, status, grp_id = query.data.split("#")
        target_id = int(grp_id)

        # Only admins may toggle the shortlink setting
        if set_type == 'is_shortlink' and query.from_user.id not in ADMINS:
            return await query.answer(
                text=(
                    f"Hᴇʏ {query.from_user.first_name}, Yᴏᴜ ᴄᴀɴ'ᴛ ᴄʜᴀɴɢᴇ sʜᴏʀᴛʟɪɴᴋ sᴇᴛᴛɪɴɢs ғᴏʀ ʏᴏᴜʀ ɢʀᴏᴜᴘ !\n\n"
                    "Iᴛ's ᴀɴ ᴀᴅᴍɪɴ ᴏɴʟʏ sᴇᴛᴛɪɴɢ !"
                ),
                show_alert=True
            )

        # Flip the boolean based on current status
        new_val = False if status == "True" else True
        await save_group_settings(target_id, set_type, new_val)

        # Reload updated settings
        settings = await get_settings(target_id)
        if settings is None:
            return await query.answer("Settings load failed", show_alert=True)

        # Build updated button list
        buttons = [
            [
                InlineKeyboardButton(
                    'Fɪʟᴛᴇʀ Bᴜᴛᴛᴏɴ',
                    callback_data=f'setgs#button#{settings["button"]}#{target_id}'
                ),
                InlineKeyboardButton(
                    'Text' if settings["button"] else 'Button',
                    callback_data=f'setgs#button#{settings["button"]}#{target_id}'
                ),
            ],
            [
                InlineKeyboardButton(
                    'Fɪʟᴇ Sᴇɴᴅ Mᴏᴅᴇ',
                    callback_data=f'setgs#botpm#{settings["botpm"]}#{target_id}'
                ),
                InlineKeyboardButton(
            ...
            [
                InlineKeyboardButton(
                    'Pʀᴏᴛᴇᴄᴛ Cᴏɴᴛᴇɴᴛ',
                    callback_data=f'setgs#file_secure#{settings["file_secure"]}#{target_id}'
                ),
                InlineKeyboardButton(
            ...
            [
                InlineKeyboardButton(
                    'Iᴍᴅʙ',
                    callback_data=f'setgs#imdb#{settings["imdb"]}#{target_id}'
                ),
                InlineKeyboardButton(
            ...
            [
                InlineKeyboardButton(
                    'Sᴘᴇʟʟ Cʜᴇᴄᴋ',
                    callback_data=f'setgs#spell_check#{settings["spell_check"]}#{target_id}'
                ),
                InlineKeyboardButton(
            ...
            [
                InlineKeyboardButton(
                    'Wᴇʟᴄᴏᴍᴇ Msɢ',
                    callback_data=f'setgs#welcome#{settings["welcome"]}#{target_id}'
                ),
                InlineKeyboardButton(
            ...
            [
                InlineKeyboardButton(
                    'Aᴜᴛᴏ-Dᴇʟᴇᴛᴇ',
                    callback_data=f'setgs#auto_delete#{settings["auto_delete"]}#{target_id}'
                ),
                InlineKeyboardButton(
            ...
            [
                InlineKeyboardButton(
                    'Aᴜᴛᴏ-Fɪʟᴛᴇʀ',
                    callback_data=f'setgs#auto_ffilter#{settings["auto_ffilter"]}#{target_id}'
                ),
                InlineKeyboardButton(
            ...
            [
                InlineKeyboardButton(
                    'Mᴀx Bᴜᴛᴛᴏɴs',
                    callback_data=f'setgs#max_btn#{settings["max_btn"]}#{target_id}'
                ),
                InlineKeyboardButton(
            ...
            [
                InlineKeyboardButton(
                    'SʜᴏʀᴛLɪɴᴋ',
                    callback_data=f'setgs#is_shortlink#{settings["is_shortlink"]}#{target_id}'
                ),
                InlineKeyboardButton(
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            ...
            # If no files were found, remove the temporary "Searching..." message
            # and present smart suggestions along with a request button.  This
            # replaces the earlier spell‑check fallback and keeps the flow
            # consistent across groups and private chats.
            if not files:
                try:
                    await m.delete()
                except Exception:
    ...
    if ENABLE_SHORTLINK and settings["button"]:
        ...
        btn = [
            [
                InlineKeyboardButton(
                    text=f"{' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('Linkz') and not x.startswith('{') and not x.startswith('Links') and not x.startswith('www'), file.file_name.split()))}",
                    url=await get_shortlink(message.chat.id, url=f"https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}")
                ),
                InlineKeyboardButton(
                    text=f"{get_size(file.file_size)}",
                    url=await get_shortlink(message.chat.id, url=f"https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}")
...
# ---------------------------------------------------------------------------
# Callback handlers for suggestions and request buttons
#
# When a user clicks on a suggestion button, we perform a fresh search
# with the selected query and display the results via the existing
# auto_filter routine.  If the user clicks on the request button, we
# optionally notify the admins or the configured request channel and
# acknowledge the request in chat.  These handlers are appended at the
# end of the module to avoid interfering with the core filter logic.

@Client.on_callback_query(filters.regex(r"^suggest#"))
async def handle_suggestion_callback(client: Client, callback: CallbackQuery):
    """
    Respond to a suggestion button press.  Only the user who initiated
    the search may use the suggestion buttons; other users will see an
    alert explaining that the suggestion is not for them.
    """
    parts = callback.data.split("#")
    # New format: suggest#<user_id>#<cache_key>#<index>
    # Old format (legacy): suggest#<user_id>#<suggestion_text>
    if len(parts) == 4:
        _, user_id, cache_key, idx = parts
        data = SUGGESTIONS_CACHE.get(cache_key)
        if not data:
            await callback.answer("Expired. Search again.", show_alert=True)
            return
        try:
            suggestion = data["suggestions"][int(idx)]
        except Exception:
            await callback.answer("Invalid selection. Search again.", show_alert=True)
            return
    elif len(parts) == 3:
        _, user_id, suggestion = callback.data.split("#", 2)
    else:
        return
    # Ensure that only the original requester can use the suggestion.
    if int(user_id) != (callback.from_user.id if callback.from_user else 0):
        await callback.answer("This suggestion isn't for you.", show_alert=True)
        return
    # Delete the suggestion message to keep the chat clean.
    try:
        await callback.message.delete()
    except Exception:
        pass
    # Perform a new search and display results.  If no results are found,
    # send a new suggestions message.
    files, offset, total_results = await get_search_results(
    ...
    if not files:
        ...
        await send_not_found_suggestions(client, callback.message, suggestion.lower(), settings)
    else:
        # Use the spoll parameter of auto_filter to bypass initial checks.
        await auto_filter(client, callback, spoll=(suggestion.lower(), files, offset, total_results))
    await callback.answer()

@Client.on_callback_query(filters.regex(r"^req#"))
async def handle_request_callback(client: Client, callback: CallbackQuery):
    """
    Respond to a request button press.  Sends a notification to the
    request channel or admins if enabled and acknowledges the user's
    request.  Only the user who initiated the search may use the
    request button.
    """
    parts = callback.data.split("#")
    # New format: req#<user_id>#<cache_key>
    # Old format (legacy): req#<user_id>#<query_text>
    if len(parts) == 3:
        _, user_id, third = parts
        if third in SUGGESTIONS_CACHE:
            data = SUGGESTIONS_CACHE.get(third)
            query = data.get("query") if data else third
        else:
            query = third
    else:
        return
    if int(user_id) != (callback.from_user.id if callback.from_user else 0):
        await callback.answer("This request isn't for you.", show_alert=True)
        return
    chat_id = callback.message.chat.id
    settings = await get_settings(chat_id)
    notify = settings.get('request_notify', True)
    # Compose the request payload with HTML formatting.  Include group
    # and user details to assist admins in handling the request.
    reporter_id = callback.from_user.id if callback.from_user else 0
    mention = callback.from_user.mention if callback.from_user else str(reporter_id)
    group_title = callback.message.chat.title or "Private"
    payload = (
        f"<b>Query Request</b>\n\n"
        f"<b>Reporter:</b> {mention} (<code>{reporter_id}</code>)\n"
        f"<b>Group:</b> {group_title} (<code>{chat_id}</code>)\n"
        f"<b>Query:</b> {query}</b>"
    )
    if notify:
        # Send the request either to the configured request channel or to
        # individual admins.  REQST_CHANNEL takes precedence.
        try:
            if REQST_CHANNEL:
                await client.send_message(chat_id=REQST_CHANNEL, text=payload, parse_mode=enums.ParseMode.HTML)
            else:
                for admin_id in ADMINS:
                    try:
                        await client.send_message(chat_id=int(admin_id), text=payload, parse_mode=enums.ParseMode.HTML)
                    except Exception:
                        continue
        except Exception:
            pass
        confirm_text = "✅ Your request has been sent to the admins. Please wait for a response."
    else:
        confirm_text = "✅ Your request has been noted. Admins were not notified."
    # Edit the message to acknowledge the request and remove the buttons.
    try:
...
# =========================
# Callback Safety Net (FIX)
# =========================
# Some deployments had "dead" inline buttons because callback queries weren't being answered
# (Telegram expects an answer quickly). This handler runs first and ACKs every callback.
@Client.on_callback_query(group=-999)
async def _cb_ack_all(client, query):
    try:
        await query.answer(cache_time=0)
        print("CB_ACK:", query.data)
    except Exception as e:
        print("CB_ACK_ERR:", repr(e))


@Client.on_callback_query(filters.regex(r"^setgs#"), group=-500)
async def _cb_setgs_toggle(client, query):
    try:
        await query.answer(cache_time=0)
        ident, set_type, status, grp_id = query.data.split("#", 3)
        grp_id = int(grp_id)

        # Admin-only guard for shortlink (if ADMINS is defined)
        if set_type == "is_shortlink":
            try:
                if query.from_user.id not in ADMINS:
                    return await query.answer("Admin only setting.", show_alert=True)
            except Exception:
                pass

        new_val = False if status == "True" else True
        await save_group_settings(grp_id, set_type, new_val)

        settings = await get_settings(grp_id)
        if not settings:
            return await query.answer("Settings load error", show_alert=True)

        # Reuse existing settings menu builder if present
        try:
            if "setting_buttons" in globals():
                kb = InlineKeyboardMarkup(setting_buttons(settings, grp_id))
            elif "settings_buttons" in globals():
                kb = InlineKeyboardMarkup(settings_buttons(settings, grp_id))
            else:
                # Minimal fallback buttons (spell_check + auto_ffilter)
                kb = InlineKeyboardMarkup([
                    [InlineKeyboardButton("Spell Check", callback_data=f"setgs#spell_check#{settings.get('spell_check')}#{grp_id}"),
                     InlineKeyboardButton("ON" if settings.get("spell_check") else "OFF", callback_data=f"setgs#spell_check#{settings.get('spell_check')}#{grp_id}")],
                    [InlineKeyboardButton("Auto Filter", callback_data=f"setgs#auto_ffilter#{settings.get('auto_ffilter')}#{grp_id}"),
                     InlineKeyboardButton("ON" if settings.get("auto_ffilter") else "OFF", callback_data=f"setgs#auto_ffilter#{settings.get('auto_ffilter')}#{grp_id}")]
                ])
            await query.message.edit_reply_markup(reply_markup=kb)
        except Exception as e:
            print("SETGS_EDIT_ERR:", repr(e))
            # Still notify user so it doesn't feel dead
            await query.answer("Saved, but UI refresh failed.", show_alert=True)
    except Exception as e:
        print("SETGS_ERR:", repr(e))
        await query.answer("Toggle failed.", show_alert=True)


@Client.on_callback_query(group=999)
async def _cb_catchall_last(client, query):
    try:
        # If something else handled it, this still harmlessly ACKs.
        await query.answer(cache_time=0)
    except Exception:
        pass
