...
async def send_not_found_suggestions(client: Client, msg, query: str, settings: dict):
    ...
    buttons = []
    # Build suggestion buttons.  Each button carries the user's ID and the
    # suggested query in its callback_data so that only the requesting user
    # can activate it.
    ...
    for suggestion in suggestions:
        buttons.append([
            InlineKeyboardButton(
                text=suggestion,
                callback_data=f"suggest#{user_id}#{suggestion}"
            )
        ])
    # Always add a request button.  The callback_data includes the user ID
    # and the original query.
    buttons.append([
        InlineKeyboardButton(
            text="Request 📩",
            callback_data=f"req#{user_id}#{query}"
        )
    ])
...
async def handle_request_callback(client: Client, callback: CallbackQuery):
    ...
    """
    try:
        _, user_id, query = callback.data.split("#", 2)
    except ValueError:
        return
    if int(user_id) != (callback.from_user.id if callback.from_user else 0):
