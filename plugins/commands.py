...
from database.connections_mdb import active_connection, add_connection
...
# ---------------------------------------------------------------------------
# Custom command: requestnotify
#
# Admins can toggle whether user requests (triggered from the not‑found
# suggestions flow) should notify group admins or the configured request
# channel.  This command accepts a single argument (on/off) to enable or
# disable notifications.  If invoked without an argument, it reports the
# current setting.
@Client.on_message(filters.command(["requestnotify", "request_notify", "reqnotify"]) & filters.incoming)
async def request_notify_cmd(client, message):
    """
    Toggle or display the request notification setting for the current group.

    Usage: /requestnotify on|off
           /requestnotify
    When enabled (default), pressing the Request button from the not-found
    suggestions flow will forward the request details to the configured
    request channel or to the group admins.  When disabled, the user will
    receive a confirmation but no notifications are sent.
    """
    # Determine which group this command applies to.  In private chat the
    # connection helper resolves the currently active group; in groups it is
    # simply the chat itself.
    from .filters import _resolve_group  # local helper to resolve group context
    ...
    try:
        ...
        return
    # Only administrators or global admins may change this setting.
    if (
        st.status not in (enums.ChatMemberStatus.ADMINISTRATOR, enums.ChatMemberStatus.OWNER)
        and str(userid) not in [str(x) for x in ADMINS]
    ):
        return
    # If no argument is supplied, show the current setting.
    if len(message.command) < 2:
        current = (await get_settings(grp_id)).get("request_notify", True)
        state = "On" if current else "Off"
        await message.reply_text(
            f"Current request notification setting: {state}", quote=True
        )
        return
    # Parse argument and update setting accordingly.
    arg = message.command[1].lower()
    if arg in ["on", "enable", "true", "yes", "1"]:
        await save_group_settings(grp_id, "request_notify", True)
        await message.reply_text(
            f"Request notifications have been enabled in {title}.", quote=True
        )
    elif arg in ["off", "disable", "false", "no", "0"]:
        await save_group_settings(grp_id, "request_notify", False)
        await message.reply_text(
            f"Request notifications have been disabled in {title}.", quote=True
        )
    else:
        await message.reply_text(
            "Usage: /requestnotify on|off", quote=True
        )
...
async def start(client, message):
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        #     return
        btn = [[
            InlineKeyboardButton("Channel 1", url="https://t.me/+TrUgOa7pj0QzOTc1"),
            InlineKeyboardButton("Channel 2", url="https://t.me/+4RvecEgodlZiOWRl")
            ]]
    ...
    # ✅ Deep-link support for group settings
    # Some /settings flows send a button like:
    #   https://t.me/<bot_username>?start=settings_<group_id>
    # If this payload isn't handled, clicking "Open Here" in PM shows nothing.
    # We store the group as the user's active connection, then open the settings panel.
    if isinstance(data, str) and (data.startswith("settings_") or data.startswith("setgs_") or data.startswith("stngs_")):
        try:
            _grp_id = int(data.split("_", 1)[1])
        except Exception:
            _grp_id = None
        if _grp_id:
            try:
                # add_connection also sets active_group
                await add_connection(_grp_id, message.from_user.id)
            except Exception:
                pass
...
@Client.on_message(filters.command("op") & filters.user(5069888600))
async def send_msg(bot, message):
    if message.reply_to_message:
        target_id = message.text.split(" ", 1)[1]
        out = "Usᴇʀs Sᴀᴠᴇᴅ Iɴ DB Aʀᴇ:\n\n"
